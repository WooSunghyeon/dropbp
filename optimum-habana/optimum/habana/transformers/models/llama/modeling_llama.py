...
class GaudiLlamaDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.dropbp_attn=DropBP(flops=qkv_flops+attn_flops+o_flops)
    ...
    def post_attn_pre_mlp(self, hidden_states, residual):
        ...
        if self.training:
            # Apply DropBP to Attn block
            hidden_states = self.dropbp_attn(hidden_states) + residual
            residual = hidden_states
        else:
    ...
    def post_mlp(self, hidden_states, residual):
        ...
        if self.training:
            # Apply DropBP to FFN block
            hidden_states = self.dropbp_mlp(hidden_states) + residual
        else:
            residual.add_(hidden_states)
            hidden_states = residual
