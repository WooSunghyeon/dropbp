...
def main(fabric: L.Fabric, data_dir: Path, checkpoint_dir: Path, out_dir: Path, drop_rate: float, is_sens_alloc: bool) -> None:
...
def train(
    ...
    for iter_num in range(1, max_iters + 1):
        ...
        def backprop():
            with fabric.no_backward_sync(model, enabled=False):
                logits = model(input_ids[:1,:], lm_head_chunk_size=128)
                # shift the targets such that output n predicts token n+1
                logits[-1] = logits[-1][..., :-1, :]
                loss = chunked_cross_entropy(logits, targets[:1, 1:])
                optimizer.zero_grad()
                fabric.backward(loss / gradient_accumulation_iters)
