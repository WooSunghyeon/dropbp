...
class Block(nn.Module):
    ...
    def forward(
        ...
        if not(self.config.pld) or not(self.training):
            n_1 = self.norm_1(x)
            ...
            if self.config.parallel_residual:
                ...
                #x = h + x
                x = self.dropbp_attn(h) + x
                
                #x = self.mlp(self.norm_2(x)) + x
                x = self.dropbp_mlp(self.mlp(self.norm_2(x))) + x
        else:
            if self.config.parallel_residual:
                raise NotImplementedError(
                        "No checkpoint amongst the ones we support uses this configuration"
                        " (non-parallel residual and progressive layer dropping)."
                    )
            else:
                if self.config.shared_attention_norm:
                    raise NotImplementedError(
                ...
                if random.random() < self.remain_p :
                    ...
                    h = self.attn(n_1, cos, sin, mask, input_pos)
                    #x = h + x
                    x = self.dropbp_attn(h)*1/self.remain_p + x
                
                #x = self.mlp(self.norm_2(x)) + x
                if random.random() < self.remain_p:
                    x = self.dropbp_mlp(self.mlp(self.norm_2(x)))*1/self.remain_p + x
