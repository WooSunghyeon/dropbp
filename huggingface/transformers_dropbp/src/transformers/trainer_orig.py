...
class Trainer:
    ...
    def __init__(
        ...
        self.end_fwd_events=[]
        self.start_bwd_events=[]
        self.end_bwd_events=[]
        self.time_warmup_steps=time_warmup_steps
        self.time_measure_steps=time_measure_steps
        ...
        #self.dropbp_handler = DropBPHandler(self.model)
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # DropBP Step 3. Adjust the drop rates of layers based on sensitivities
                def backprop(inputs=inputs):
                    ...
                    with self.compute_loss_context_manager():
                        loss = self.compute_loss(model, tiny_inputs)
                ...
                if step == int(max_steps*args.gradient_accumulation_steps*0.1) and args.drop_rate != 0 and not(self.measure_time_memory):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], step: int) -> torch.Tensor:
        ...
        if self.measure_time_memory and step>self.time_warmup_steps:
            ## make fwd event
            start_fwd_event = torch.cuda.Event(enable_timing=True)
            end_fwd_event = torch.cuda.Event(enable_timing=True)
            ...
            ## measure fwd time
            start_fwd_event.record()
            with self.compute_loss_context_manager():
                ## measure activation memory
                before_act_save = torch.cuda.memory_allocated(self.args.device)/1e9
                loss = self.compute_loss(model, inputs)
                after_act_save = torch.cuda.memory_allocated(self.args.device)/1e9
                act_mem = after_act_save-before_act_save
            end_fwd_event.record()
            
            ## append fwd event
            self.start_fwd_events.append(start_fwd_event)
            self.end_fwd_events.append(end_fwd_event)
            ...
            ## make bwd event
            start_bwd_event = torch.cuda.Event(enable_timing=True)
            end_bwd_event = torch.cuda.Event(enable_timing=True)
            
            ## measure bwd time
            start_bwd_event.record()
            ...
            model_state_mem = torch.cuda.memory_allocated(self.args.device)/1e9 ## model sate memory: param + grad + optim
            end_bwd_event.record()
            
            ## append bwd event
            self.start_bwd_events.append(start_bwd_event)
            self.end_bwd_events.append(end_bwd_event)
            ## report results
            if self.measure_time_memory and step == self.time_warmup_steps + self.time_measure_steps:
                torch.cuda.synchronize()
                forward_time = [s.elapsed_time(e) for s, e in zip(self.start_fwd_events, self.end_fwd_events)]
                backward_time = [s.elapsed_time(e) for s, e in zip(self.start_bwd_events, self.end_bwd_events)]
                print("======================================================================")
                print("forward time: ", round(sum(forward_time) / len(self.start_fwd_events), 2), "ms")
                print("backward time: ", round(sum(backward_time) / len(self.start_fwd_events), 2), "ms")
                print("total time: ", round((sum(forward_time) + sum(backward_time)) / len(self.start_fwd_events), 2), "ms")
                print("======================================================================")
                print("model state memory: ", round(model_state_mem, 2), "GB")
                print("activation memory: ", round(act_mem, 2), "GB")
                print("total_memory:", round(model_state_mem + act_mem, 2), "GB" )
